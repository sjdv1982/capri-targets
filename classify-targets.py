...
def select_matches(target, pdbs):
    pdbs = set(pdbs)
    result = {}
    for entity_name, entity in target.items():
        result[entity_name] = []
        for match in entity["matches"]:
            ...
            if jmatch["pdb"] in pdbs:
                result[entity_name].append(match)
    return result
...
def filter_all_targets(targets):
    ...
    for target_name, target in targets.items():
        ...
        if len(template_groups):
            coverage = detect_coverage(template_groups)
        else:
            coverage = {entity: "bad" for entity in target}        
        ...
        if len(close_template_groups):
            close_coverage = detect_coverage(close_template_groups)
        else:
            close_coverage = {entity: "bad" for entity in target}
        ...
        if any_bad:
            ...
            if len(trivial_pdbs):
                ...
                if len(trivial_pdbs) == 1:
                    ...
                    my_result["matches"] = select_matches(target, trivial_pdbs)
                else:
